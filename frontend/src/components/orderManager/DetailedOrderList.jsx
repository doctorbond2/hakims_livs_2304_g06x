...
function DetailedOrderList({ orderId, onClose }) {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      if (orderId) {
        try {
          const fetchedOrderDetails = await GET_REQUEST(`/api/order/${orderId}`);
    ...
    <>
      {orderDetails && ( 
        <div>
          ...
          <p><strong>Status:</strong> {orderDetails.status.paid ? 'Paid' : 'Not Paid'}, {orderDetails.status.shipped ? 'Shipped' : 'Not Shipped'}</p>
