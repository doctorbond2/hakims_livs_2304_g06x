...
const CartTotal = () => {
  ...
  useEffect(() => {
    ...
    cartData.forEach((item) => {
      ...
      const itemSavings =
        (item.price - item.discountedPrice) * item.cartQuantity;
    ...
    <shad.Card className="w-96 h-32">
      <div className="m-5">
        <div className="text-xl">
          Totalsumma {totalPriceAfterTax.toFixed(2).replace(".", ",")} kr
        </div>
        <div className="text-gray-500">
          Moms (12%): {taxAmount.toFixed(2).replace(".", ",")} kr
        </div>
        <div className="text-red-600">
          Du sparar {savings.toFixed(2).replace(".", ",")} kr!
        </div>
      </div>
    </shad.Card>
  );
